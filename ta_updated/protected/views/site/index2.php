...
<body id="body" onLoad="getDataInit(defaultX,defaultY,defaultParameter,defaultEdge)">
	...
	<script id="scriptInit" type="text/javascript">
		...
		function redraw(dataString) {
			...
			dataX = data2.data3;
			console.log(dataX);
			console.log(dataX['links'][0]);
			
			dataX.nodes = getChildren(dataX.nodes);

			// Setting data untuk link, source dan targetnya ud data node
			if(dataX.links.length != 0) {
				var counter_rlink;
				counter_rlink = 0;
				var rlinks = new Array(dataX.links.length);
				for(var i = 0; i < dataX.links.length; i++) {
					var j, k, l, m;
					j = 0; k = 0;
					
					var sudah_ketemu; sudah_ketemu = 0;
					while(dataX.nodes.length > j && !sudah_ketemu) {
						if(dataX.nodes[j].id.length == 1 && dataX.links[i].source != dataX.nodes[j].id[0]) {
							j++;				
						}
						else if(dataX.nodes[j].id.length == 1 && dataX.links[i].source == dataX.nodes[j].id[0]) {
							sudah_ketemu = 1;
						}
						else {
							l = 0;
							
							while(dataX.nodes[j].id.length > l && dataX.links[i].source != dataX.nodes[j].id[l]) { // 3>0 && 1!=1
								l++;
							}

							if(dataX.nodes[j].id.length < l || dataX.links[i].source != dataX.nodes[j].id[l]) {
								j++;
							}
							else if (dataX.nodes[j].id.length > l && dataX.links[i].source == dataX.nodes[j].id[l]) {
								sudah_ketemu = 1;
							}
						}
					}
					...
					while(dataX.nodes.length>k && !sudah_ketemu) {
						console.log(dataX.nodes[k]);
						if(dataX.nodes[k].id.length == 1 && dataX.links[i].target != dataX.nodes[k].id[0]) {
							k++;
						}
						else if (dataX.nodes[k].id.length == 1 && dataX.links[i].target == dataX.nodes[k].id[0]) {
							sudah_ketemu = 1;
						}
							...
							while(dataX.nodes[k].id.length > m && dataX.links[i].target != dataX.nodes[k].id[m]) {
								m++;
							}
							
							if(dataX.nodes[k].id.length < m || dataX.links[i].target != dataX.nodes[k].id[m]) {
								k++;
							}
							else if(dataX.nodes[k].id.length > m && dataX.links[i].target == dataX.nodes[k].id[m]) {
								sudah_ketemu = 1;
							}
						}
					}
					...
					if(j < dataX.nodes.length && k < dataX.nodes.length && ((dataX.nodes[j].id.length == 1 && dataX.nodes[k].id.length == 1 && dataX.links[i].target == dataX.nodes[k].id && dataX.links[i].source == dataX.nodes[j].id) || (dataX.nodes[j].id.length > 1 && dataX.nodes[k].id.length > 1 && dataX.links[i].target == dataX.nodes[k].id[m] && dataX.links[i].source == dataX.nodes[j].id[l]) || (dataX.nodes[j].id.length == 1 && dataX.nodes[k].id.length > 1 && dataX.links[i].target == dataX.nodes[k].id[m] && dataX.links[i].source == dataX.nodes[j].id) ||(dataX.nodes[j].id.length > 1 && dataX.nodes[k].id.length == 1 && dataX.links[i].target == dataX.nodes[k].id && dataX.links[i].source ==dataX.nodes[j].id[l]))) {
						
						rlink[counter_rlink]=new Array();
						rlink[counter_rlink].source=dataX.nodes[j];
						
						rlink[counter_rlink].target=dataX.nodes[k];
						counter_rlink++;
					} else {}
				}
			}
			...
			if ($("#sumbuX option:selected").text()=='Tahun Publikasi') {
				//dataX.nodes=dataX.nodes;
				//var i;
				for(var i = 0; i < dataX.nodes.length; i++) {
					dataX.nodes[i].sumbu_x=parseInt(dataX.nodes[i].sumbu_x);				
				}

				x = d3.scale.ordinal()			
				.domain(dataX.nodes.sort(function(a, b){  return d3.ascending(a.sumbu_x, b.sumbu_x)}).map(function(d) { return d.sumbu_x; }))
				.rangeRoundBands([0, width], .1);
			}
			...
			// Sorting huruf
				...
				for(i=0; i<dataX.nodes.length; i++) {
					dataX.nodes[i].sumbu_x=dataX.nodes[i].sumbu_x.charAt(0).toUpperCase() + dataX.nodes[i].sumbu_x.slice(1);
				}
				x = d3.scale.ordinal()
				.domain(dataX.nodes.sort(function(a, b){  return d3.ascending(a.sumbu_x, b.sumbu_x)}).map(function(d) { return d.sumbu_x; }))
				.rangeRoundBands([0, width], .1);
			...
			/*
			y = d3.scale.linear()
					.domain([d3.min(dataX.nodes.map(function(d) {return d.sumbu_y; }))-5, d3.max(dataX.nodes.map(function(d) {return d.sumbu_y; }))])
					.range([0, height]);*/
				//ubah angka string jd angka numeric
				
				for(var i=0;i<dataX.nodes.length;i++) {
					dataX.nodes[i].sumbu_y=parseInt(dataX.nodes[i].sumbu_y);				
				}

				y = d3.scale.ordinal()
				.rangeRoundBands([height, 0], .1)
				.domain(dataX.nodes.sort(function(a, b){  return d3.ascending(a.sumbu_y, b.sumbu_y)}).map(function(d) { return d.sumbu_y; }));
			} else {
				for(i=0; i<dataX.nodes.length; i++) {
					dataX.nodes[i].sumbu_y=dataX.nodes[i].sumbu_y.charAt(0).toUpperCase() + dataX.nodes[i].sumbu_y.slice(1);
				}
			...
			var start;
			if(minimum/2 < 15) {
				alert("Data yang dimasukkan terlalu banyak! Kurangi dataX");
				...
				if(d3.min(dataX.nodes.map(function(d) {return d.id.length; })) != d3.max(dataX.nodes.map(function(d) {return d.id.length; }))) {
					start = 15;
				} else {
					start = minimum/2;
				}
			...
			.domain([d3.min(dataX.nodes.map(function(d) {return d.id.length; })), d3.max(dataX.nodes.map(function(d) {return d.id.length; }))])
			...
			var keyword=new Array(dataX.nodes.length);
			//keyword[0]=[];
			for(i = 0; i < dataX.nodes.length; i++) {
				//string=dataX.nodes[i].keyword[0].split(" ");
				keyword[i]=new Array();
				keyword[i]=dataX.nodes[i].keyword[0].replace(/ /g,"\n");;
				dataX.nodes[i].keyword=[];
				//dataX.nodes[i].keyword.splice(0, 1, dataX.nodes[i].keyword[0].split(" "));
				$.merge(dataX.nodes[i].keyword, keyword[i]);
			}
			...
			var g1 = chart.selectAll("g.circle").data(dataX.nodes);

			// chart.selectAll("g.circle").dataX(dataX.nodes).enter().append("circle")
			...
			if(rlink.length != 0) {
				// Untuk membuat panah
				var marker = chart.selectAll("g.marker").data(dataX.links)
